...
public class RobotContainer {
  ...
  private void initializeCamera() {
    ...
    if (enumerateSources.length > 0 && enumerateSources[0].getName().contains("USB")) {
      Shuffleboard.getTab("RobotData2").add("Camera", enumerateSources[0]).withPosition(5, 0).withSize(3, 3)
          .withWidget(BuiltInWidgets.kCameraStream);
    }
    HttpCamera limelight = new HttpCamera("Limelight", "http://10.43.29.11:5800");
    CameraServer.startAutomaticCapture(limelight);

    Shuffleboard.getTab("RobotData2").add("Limelight Camera", limelight).withPosition(2, 0).withSize(2, 2)
        .withWidget(BuiltInWidgets.kCameraStream);
  }
  ...
  private void configureAutoChooser(Drivetrain drivetrain) {
    ...
    Shuffleboard.getTab("RobotData2").add("SelectAuto", m_chooser).withSize(2, 1).withPosition(3, 2);
